                ...
                <div className="product_container">
                    <div className="product_details-wrapper">
                        <div className="product_preview">
                            ...
                            <img src={`../${data.image}`} alt={data.name} className="product_image" />
                        </div>
                        <div className="product_info">
                            <h1>{data.name}</h1>
                            <div className="product_desc-wrapper">
                                <p className="product_desc">Диагональ экрана {data.name} равна 32 дюймам (80 см). Благодаря этому вы сможете закрепить его на стене в небольшой комнате, при этом оптимальное расстояние до экрана будет достигать 2-2,5 метров. <Link href="#details"><a>О товаре</a></Link></p>
                            </div>
                            <div className="product_cart-wrapper">
                                <div className="product_price">{`${data.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")} сум`}</div>
...
export async function getStaticProps({ params }) {
    const res = await fetch(process.env.API_URL)
    const products = await res.json()
    const data = products.find(x => x.name.toLowerCase().replace(/\s+/g, '-') === params.id);
    return {
        props: { data, products }
    }
}

...
export default function Category({products, categoryId, query, category}) {
    ...
    useEffect(() => {
        setBrandData(brandMetaFunction)
    }, [filterBrands])


    const checkBrandFunction = () => {
        let arr = []

        for (let i = 0; i < brands.length; i++) {
            for (let j = 0; j < filterBrands.length; j++) {
                if (brands[i].id === filterBrands[j]) {
                    arr.push(brands[i])
                }
            }
        }
    ...
    const {arr} = checkBrandFunction()

    const brandMetaFunction = () => {
        let description = []
        let tags = []
        if (arr) {
            for (let i = 0; i < arr.length; i++) {
                description.push(arr[i].meta.description)
                tags.push(arr[i].meta.title)
            }
        }
        return {
            description,
    ...
    return (
        <>
            <SEO
                ...
                description={`${category.meta.description || category.description} ${brandData.hasOwnProperty("description") ? brandData.description.join() : ""}`}
                image={category.image}
                keywords={`${category.meta.tags} ${brandData.hasOwnProperty("tags") ? brandData.tags.join() : ""}`}
            />
